...
- `GET /api/paper/runs` and `GET /api/paper/runs/{id}` for the signed-in user's newest-first paper run journal and full replay payloads.
...
- Backtesting page paper journal: signed-in users can run a simulated paper allocation, review recent run history, open run details, and load an older run into the visible Paper Equity view without broker integration.
