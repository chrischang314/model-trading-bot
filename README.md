...
- `GET /api/paper/runs` and `GET /api/paper/runs/{id}` for the signed-in user's newest-first paper trading run journal and full replay payloads.
...
- Backtesting page paper journal: each successful paper run is saved as a user-scoped record with requested symbols, strategy, cash, resulting equity, positions, and orders; older runs can be loaded into the visible paper view for review without placing orders.
